...
import { addVariant, updateVariant, getImageUrl } from '../utils/api';
...
export default function VariantModal({ productId, variant, onClose, onSuccess }) {
  ...
  const [photos, setPhotos] = useState([]); // [{file, preview, description}]
  const [currentIndex, setCurrentIndex] = useState(0);
  const [step, setStep] = useState('pick'); // 'pick' | 'describe'
        ...
        <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 460, maxHeight: '90vh', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div>
              ...
              <div style={{
                borderRadius: 10, overflow: 'hidden', border: '1.5px solid rgba(201,168,76,0.3)',
                marginBottom: 10, background: 'rgba(0,0,0,0.2)',
              }}>
    ...
    <div className="modal-overlay" onClick={e => e.stopPropagation()}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{
          ...
          {photos.map((_, i) => (
            <div
              ...
              style={{
                ...
                height: 8,
                borderRadius: 4,
                background: i === currentIndex ? 'var(--gold)' : i < currentIndex ? 'rgba(201,168,76,0.5)' : 'rgba(201,168,76,0.2)',
                cursor: 'pointer',
              ...
              {currentIndex > 0 && (
                <button
                  ...
                  style={{
                    ...
                    background: 'rgba(61,13,24,0.8)', border: '1px solid rgba(201,168,76,0.4)',
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <textarea
            className="input"
            placeholder="e.g. Deep red Banarasi silk with gold zari border..."
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {currentIndex > 0 && (
            <button className="btn btn-outline" onClick={() => goTo(currentIndex - 1)} style={{ flex: 1 }}>
              ‹ Prev
            </button>
          )}

          {!allDone ? (
            <button className="btn btn-gold" onClick={() => goTo(currentIndex + 1)} style={{ flex: 2, justifyContent: 'center' }}>
              Next ›
            </button>
          ) : (
            <button
            ...
            >
              {loading ? (
                <>
                  ...
                  Uploading {uploadProgress}/{photos.length}...
                </>
              ) : (
        ...
        {!loading && (
          <button
            className="btn"
            onClick={() => { setStep('pick'); setPhotos([]); }}
            style={{ width: '100%', justifyContent: 'center', marginTop: 8, color: 'rgba(250,243,224,0.35)', background: 'none', fontSize: '0.8rem' }}
          >
            ✕ Cancel / Start Over
          </button>
        )}
